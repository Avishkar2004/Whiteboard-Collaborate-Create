...
const Register = () => {
  ...
  const testConnection = async () => {
    try {
      ...
      const dbStatusResponse = await api.get('/api/db-status');
      console.log('Database status response:', dbStatusResponse.data);
      ...
      const dbStatus = dbStatusResponse.data.database === 'connected' ? 'Connected' : 'Disconnected';
      setTestResult(`Connection successful! Health: ${healthResponse.data.status}, API: ${apiResponse.data.message}, Database: ${dbStatus}`);
    } catch (error) {
